import { Client, Account, Databases, ID } from "appwrite";

const client = new Client();

const appwrite_api_key = '65fdbf643f997e6ea8f1';
// const database_key = '';
const database_id = '65fead4a0d50fe942440';

// Set the endpoint URL and project ID for the client
client
  .setEndpoint("https://cloud.appwrite.io/v1")
  .setProject(appwrite_api_key);

// Create a new account instance using the client
export const account = new Account(client);

// Create a new databases instance using the client and database ID
export const databases = new Databases(client);

export const createPlan = (user_id,planstring) => databases.createDocument(
  database_id,
  '65feae246cd725f25e0d', //connection id for : My Plans
  ID.unique(),
  { "user": user_id, "planstring": planstring }
);

// createContainer.then(function (response) {
//   console.log(response);
// }, function (error) {
//   console.log(error);
// });
